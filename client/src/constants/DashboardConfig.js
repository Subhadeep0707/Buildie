export const DASHBOARD_SECTIONS = [
  {
    categoryTitle: "Analytics & Costing",
    items: [
      { id: "boq", label: "BOQ & Cost Chart" },
      { id: "breakdown", label: "Room Breakdown" },
      { id: "rates", label: "Material Rates" },
    ],
  },
  {
    categoryTitle: "Core Structure",
    items: [
      { id: "structural", label: "Structural Details" },
      { id: "earthwork", label: "Earthwork" },
      { id: "openings", label: "Rooms & Openings" },
      { id: "detailedRooms", label: "Detailed Rooms Design" },
      { id: "finishing", label: "Finishing" },
    ],
  },
  {
    categoryTitle: "MEP & Custom Services",
    items: [
      { id: "plumbing", label: "Plumbing" },
      { id: "electricity", label: "Electricity" },
      { id: "stair", label: "Staircase" },
      { id: "septic", label: "Septic Tank" },
      { id: "rwh", label: "Rain Water Harvesting" },
      { id: "solar", label: "Solar Rooftop" },
      { id: "lift", label: "Lift Floorwise" },
      { id: "pump", label: "Pump Capacity" },
      { id: "firefighting", label: "Firefighting" },
    ],
  },
];
